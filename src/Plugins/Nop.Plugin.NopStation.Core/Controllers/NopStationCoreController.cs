...
namespace Nop.Plugin.NopStation.Core.Controllers
{
    public class NopStationCoreController : NopStationAdminController
    {
        ...
        [EditAccessAjax, HttpPost, License]
        ...
        public async Task<IActionResult> PluginInfo()
        {
            ...
            var assemblies = AppDomain.CurrentDomain.GetAssemblies().Where(x => x.FullName.Contains(".NopStation.") && !x.GetName().Name.EndsWith(".Views")).ToList();
            foreach (var assembly in assemblies)
            {
                var assemblyName = assembly.GetName();
                var filePath = assemblyName.CodeBase["file:///".Length..].Replace("/", "\\");
